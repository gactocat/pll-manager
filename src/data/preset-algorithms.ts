...
//
// Post-processing applied to the wiki source:
//
// 1. Leading y-axis rotation in the opening parenthesised group is
//    rewritten as a U-layer turn so splitAuf can pick it up as AUF.
//    From the U-face viewpoint y and U produce the identical visual
//    rotation, so the algorithm is functionally equivalent for PLL.
//      "(y) [body]"    -> "(U) [body]"
//      "(y2) [body]"   -> "(U2) [body]"
//      "(y') [body]"   -> "(U') [body]"
//      "(y x) [body]"  -> "(U) x [body]"   // compound: extract y,
//      "(y2 z) [body]" -> "(U2) z [body]"  // leave sibling rotation
//      "(y' z) [body]" -> "(U') z [body]"  // (without its old parens)
//
// 2. Decorative parentheses are stripped. Only two kinds of parens
//    remain in the data:
//      - leading AUF: "(U)", "(U2)", "(U')"
//      - repeat notation: "(group)2", "(group)5"
//    All other parens (move grouping, trailing setup-undo, etc.) are
//    removed so the algorithm reads as a flat sequence of moves.
